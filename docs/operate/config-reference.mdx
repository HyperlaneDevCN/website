...
**描述：** 链上跨链燃气支付主合同的地址。另请参见 [合约地址](../reference/contract-addresses.mdx)。
...
- `"fallback"` 将尝试第一个 URL，然后在连接或其他提供者故障时自动"回退"到下一个提供者；这仅应重试由于提供者导致的调用失败，而不是调用本身的失败；例如，燃气不足错误将不会回退。
...
**描述：** 按优先级排序的燃气支付强制配置的 JSON 字符串化数组。最后一个策略应为捕获所有策略，处理未匹配到先前策略的任何消息。
