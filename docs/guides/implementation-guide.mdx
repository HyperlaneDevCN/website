...
1. [合约](#1-contracts)：为应用程序开发者提供发送和接收消息的接口
2. [代理](#2-agents)：通过增加安全性和中继消息来操作协议
3. [应用程序](#3-applications)：使用该协议并展示其能力的应用程序
...
燃气支付者用于支付目标链上消息处理所需的燃气。这在中继器愿意补贴消息处理时并不是严格要求的。

:::warning

实施对足够燃气支付的强健检查，考虑链特定的代币处理。我们的 Solidity 实现收取本地消息值，但对于其他链，您可能需要收取特定代币，并相应地调整燃气开销和 `tokenExchangeRate`。
...
event DestinationGasConfigSet(
    ...
    address gasOracle,   // 燃气预言机
    uint96 gasOverhead   // 目标燃气开销
);
```
...
中继器还可能要求在处理目标链上的消息之前，先在原链上为特定消息 ID 支付燃气。为此，他们必须部署一个 IGP，并将其地址设置为受益人，并索引燃气支付事件。请参见 [燃气支付强制执行特性](https://github.com/hyperlane-xyz/hyperlane-monorepo/blob/main/rust/main/agents/relayer/src/msg/gas_payment/mod.rs)。我们建议从不强制燃气支付政策开始，然后逐渐支持更严格的政策。
